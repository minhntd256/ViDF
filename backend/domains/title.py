...
logger = logging.getLogger("fakemark.domains.title")

TITLE_COMBINED_PROMPT = """You are a fact-checking assistant. You are given a list of factual claims extracted \
from a video title. For EACH claim, INDEPENDENTLY:
1. Reason briefly using your own knowledge (state clearly if you cannot verify it).
2. Decide whether it is false or unsupported (this is the "false_title" misinformation type). A claim being \
false does not make the others false — judge every claim on its own.
...
TITLE_DECISION_FROM_TAVILY_PROMPT = """You are a fact-checking classifier.
...
def run_title_domain(
    llm: QwenClient,
    title: str,
    claims: List[str],
    tavily: Optional[TavilyClient] = None,
) -> DomainResult:
    result = DomainResult(domain="title", claims=claims)

    if not (title or "").strip():
        result.analysis_text = "(no title provided)"
        result.reasoning = "No title provided — nothing to evaluate."
        return result

    if not claims:
        result.analysis_text = "(no checkable claims extracted)"
        result.reasoning = "Title has no checkable factual content — nothing to evaluate."
        return result
    ...
    if tavily is not None and tavily_ok:
        ...
        prompt = TITLE_DECISION_FROM_TAVILY_PROMPT.format(
        ...
        if tavily is not None and not tavily_ok:
            logger.warning("[title] Tavily unavailable (quota/error) — falling back to Qwen's own knowledge")
        prompt = TITLE_COMBINED_PROMPT.format(claims_block=_claims_block(claims))
        raw = llm.generate(prompt, json_mode=True)
        parsed = try_parse_json(raw) or {}
    ...
    if fake_claims:
        result.fake_types = ["false_title"]
        result.fake_claims = fake_claims
        result.text_groundings["false_title"] = fake_claims
        logger.info("[title] FAKE — fake_claims=%r reasoning=%r", fake_claims, result.reasoning)
    else:
        logger.info("[title] real — reasoning=%r", result.reasoning)
